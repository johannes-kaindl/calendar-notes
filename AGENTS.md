...
- UI (`src/obsidian/command-modal.ts` `SchemaFormModal`, `src/obsidian/plan-preview-modal.ts`): Formular aus Schema (Text/Toggle/Dropdown/TextArea/Datum, Email-Felder mit Kontakt-Suggester), Diff-Vorschau vor jeder Ausführung, Kommandos `command-run`/`command-new-event`/`command-new-contact`/`command-undo`/`command-push-hand-edits`.
- GUI-Smoke um P10–P13 erweitert (`scripts/gui-smoke.ts`, nur `--section generic`; Live-Lauf 2026-08-23: generic 8/11, pallas 4/4, keine Regression, Protokoll `docs/smoke/baseline-2026-08-23.md`): P10 Kommando via API (`event.move`), P11 Einladung ohne Scheduling/Transport (Route `ics`), P13 API-Lesen (`events`/`contacts`/`tools().length === commands().length`). **P10/P11/P13 sind aktuell ✘ — ECHTER Plugin-Bug, nicht Treiber-Bug:** `src/main.ts` ruft die core `registerCommands()` (`src/core/commands/registry.ts`) nirgends mit `EVENT_COMMANDS`/`CONTACT_COMMANDS` auf — nur Tests befüllen die Registry manuell. Zur Laufzeit ist `commandRegistry()` leer: `api.commands()`/`api.tools()` liefern `[]`, `api.plan(id, …)` liefert für jede id `{error:"command-not-found"}`. Fix-Hypothese: in `onload()` vor `this.commandFlow = new CommandFlow(...)` (Zeile ~119) einmalig `registerCommands([...EVENT_COMMANDS, ...CONTACT_COMMANDS])` mit Guard aufrufen — **nicht** in Task 8 behoben (Scope), Follow-up-Task nötig. P12 **übersprungen** — `undo` hat (noch) keinen headless-Pfad: `CommandFlow.undoLast()`/`runUndo()` brauchen eine aktive Notiz und öffnen immer die `PlanPreviewModal`, die API kennt kein `undo()`, `undo.last` steht nicht in der `commandRegistry()`. ⚠ P12 übersprungen: kein programmatischer Undo-Pfad; Follow-up: `undo.last` in die Registry + API.
- Hinweis für M5: `event.set-title` liest/schreibt `SUMMARY` direkt; ein künftiges Lokalisierungs-Feature müsste `titleKey`/`summaryKey` als eigene, vom Profil unabhängige Felder einführen, falls Termin-Titel je Sprache getrennt geführt werden sollen — aktuell gibt es dafür keinen Mechanismus.
- 439 Unit-Tests + 4 Integration-Tests (0 Warnings)
